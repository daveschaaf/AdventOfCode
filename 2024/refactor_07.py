...
def calculate_part_1(data, part = 1):
    result = 0
    for line in data:
        target = line[0]
        numbers = line[1]
        calibrated_numbers = calibrate_pair(numbers[0], numbers[1],  part = part)
        for raw_num in numbers[2:]:
            recalibrated = []
            for calced_num in calibrated_numbers:
                recalibrated = recalibrated + calibrate_pair(calced_num, raw_num, part = part)
            calibrated_numbers = recalibrated
            print(f"{len(calibrated_numbers)}")
        if target in calibrated_numbers:
            result += target
    return result

def calculate_part_2(data):
    return calculate_part_1(data, part = 2)
